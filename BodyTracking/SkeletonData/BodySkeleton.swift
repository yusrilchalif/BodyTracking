...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARBodyAnchor) {
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            ...
            switch jointName {
            case "neck_1_joint", "neck_2_joint", "neck_3_joint", "neck_4_joint", "head_joint", "left_1_shoulder_joint", "right_shoulder_1_joint":
                jointRadius *= 0.5
            case "jaw_joint", "chin_joint", "left_eye_joint", "left_eyeLowerLid_joint", "left_eyeUpperLid_joint", "right_eyeUpperLid_joint", "right_eyeball_joint":
                jointRadius *= 0.2
                jointColor = .yellow
            ...
            case _ where jointName.hasPrefix("left_hand") || jointName.hasPrefix("right_hand"):
                jointRadius *= 0.25
                jointColor = .yellow
            case _ where jointName.hasPrefix("left_toes") || jointName.hasPrefix("right.toes"):
                jointRadius *= 0.5
                jointColor = .yellow
            default:
                jointRadius = 0.05
    ...
    func update(with bodyAnchor: ARBodyAnchor) {
        ...
        for bone in Bones.allCases {
            ...
            guard let entity = bones[boneName],
                  let skeletoneBone = createSkeletoneBone(bone: bone, bodyAnchor: bodyAnchor)
            else { continue }
            
            entity.position = skeletoneBone.centerPos
            entity.look(at: skeletoneBone.toJoint.position, from: skeletoneBone.centerPos, relativeTo: nil)
        }
    }
    
    private func createJoint(radius: Float, color: UIColor = .white) -> Entity {
        let mesh = MeshResource.generateSphere(radius: radius)
        let material = SimpleMaterial(color: color, roughness: 0.08, isMetallic: true)
    ...
    private func createSkeletoneBone(bone: Bones, bodyAnchor: ARBodyAnchor) -> SkeletonBone? {
        ...
        let fromJoint = SkeletonJoint(name: bone.jointFromName, position: jointToEntityPosition)
    ...
    private func createBoneEntity(for skeletonBone: SkeletonBone, diameter: Float = 0.04, color: UIColor = .white) -> Entity {
        let mesh = MeshResource.generateBox(size: [diameter, skeletonBone.length], cornerRadius: diameter/2)
